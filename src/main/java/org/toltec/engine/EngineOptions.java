...
public class EngineOptions {
    ...
    /**
     * Milliseconds between game-logic ticks — this drives {@code tick()},
     * including {@link org.toltec.unit.Unit}'s per-cell movement interpolation
     * ({@code stepAlongPath()}).
     * <p>
     * Kept close to {@link #renderIntervalMs} on purpose: movement progress
     * is computed from wall-clock elapsed time (see {@code Unit.moveStepStartNs}),
     * so a coarse tick rate doesn't change how <em>fast</em> a unit walks —
     * only how often its on-screen position is refreshed. If this is much
     * larger than {@code renderIntervalMs} (e.g. the old default of 100ms
     * against a 16ms render loop), the canvas repaints several times between
     * two position updates and shows the same frozen frame, then jumps in one
     * visible step to the next — this reads as stutter/"jerk", most obvious
     * right at cell borders where the per-tick step is largest. Lowering this
     * to roughly match (or beat) {@code renderIntervalMs} fixes that without
     * touching any movement math.
     */
    public long tickIntervalMs   = 16;
